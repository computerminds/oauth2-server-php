    ...
    UserInfoControllerInterface
{
    ...
    protected function createDefaultAuthorizeController()
    {
        ...
        $config = array_intersect_key($this->config, array_flip(explode(' ', 'allow_implicit enforce_state require_exact_redirect_uri enforce_pkce'))); //[pkce]
        if ($this->config['use_openid_connect']) {
    ...
    protected function createDefaultIdTokenResponseType()
    {
        ...
        $config = array_intersect_key($this->config, array_flip(explode(' ', 'issuer id_lifetime jwt_extra_payload_callable')));     //[dnc13']
